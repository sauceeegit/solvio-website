...
const bestsellersTh = [
  {
    specs: [
      '2 แผงโซลาร์ขนาด 450 W ต่อแผง รวม 900 Wp',
      'เลือกตำแหน่งติดตั้งได้เอง',
      'ขยายได้กับ Solvio Venus A ขนาด 2.12–6.36 kWh',
    ],
  },
  {
    specs: [
  ...
  {
    specs: [
      '4 แผงโซลาร์ขนาด 450 W ต่อแผง รวม 1800 Wp',
      'เลือกตำแหน่งติดตั้งได้เอง',
      'ขยายได้กับ Solvio Venus A ขนาด 2.12–6.36 kWh',
    ],
  },
    ...
    <section id="bestsellers" className="scroll-mt-20 pb-16 pt-8 sm:pb-24 sm:pt-12" style={{ backgroundColor: '#f5f5f7' }}>
      <div className="container-x">
        <Reveal>
          <div className="flex items-end justify-between gap-4 mb-10">
            <h2 className="text-2xl font-semibold tracking-tight text-ink sm:text-3xl" style={{ fontFamily: '"Space Grotesk", system-ui, sans-serif' }}>
              {lang === 'th' ? 'ระบบที่ได้รับความนิยมสูงสุด' : 'Our most popular systems'}
            </h2>
        ...
        <div className="flex snap-x gap-4 overflow-x-auto pb-4 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:grid sm:gap-5 sm:overflow-visible sm:pb-0 sm:grid-cols-2 lg:grid-cols-3" onPointerLeave={() => setHoveredIdx(null)}>
          {bestsellers.map((p, i) => {
            const thSpecs = lang === 'th' ? bestsellersTh[i]?.specs : null;
              ...
              <Reveal key={p.id} delay={i * 0.06} className="max-sm:w-[78%] max-sm:shrink-0 max-sm:snap-start">
                ...
                >
                  ...
                  <div className="flex flex-1 flex-col gap-4 p-5">
                    <div>
                      <p className="font-mono text-[10px] uppercase tracking-widest text-ink/70">{lang === 'th' ? 'โซลาร์ระเบียง' : (p.tagline || 'Balcony Solar')}</p>
                      <h3 className="mt-1 text-xl font-semibold text-ink" style={{ fontFamily: '"Space Grotesk", system-ui, sans-serif' }}>{p.name}</h3>
                    </div>

                    <ul className="flex flex-col gap-1.5">
                      {(thSpecs || p.specs).map((s) => (
                    ...
                    <div className="mt-auto flex items-end justify-between gap-3 pt-4 border-t border-black/[0.06]">
                      <div>
                        <p className="font-mono text-[10px] uppercase tracking-wider text-ink/40">{lang === 'th' ? 'เริ่มต้น' : 'From'}</p>
                      ...
                      >
                        {lang === 'th' ? 'ดูรายละเอียด' : 'See details'} <ArrowRight size={15} />
                      </Link>
                    </div>
                  </div>
                </div>
