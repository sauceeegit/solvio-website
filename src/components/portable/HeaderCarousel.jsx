...
export default function HeaderCarousel({ slides, interval = 3375 }) {
  ...
  // Desktop auto-advance — pause longer on video slides.
  useEffect(() => {
    if (slides.length <= 1) return;
    const current = slides[i];
    const delay = current?.video ? (videoRefs.current[i]?.duration * 1000 || 6000) : interval;
    const t = setTimeout(() => setI((p) => (p + 1) % slides.length), delay);
    return () => clearTimeout(t);
  }, [i, slides, interval]);

  // Play/pause video slides on desktop when they become active.
  useEffect(() => {
    slides.forEach((s, idx) => {
      const vid = videoRefs.current[idx];
      if (!vid) return;
      if (idx === i) { vid.currentTime = 0; vid.play(); }
      else vid.pause();
    });
  }, [i, slides]);
    ...
    <>
      ...
      <div className="relative sm:hidden">
        <div
          ref={trackRef}
          onScroll={onScroll}
          className="no-scrollbar flex aspect-[3/4] w-full snap-x snap-mandatory overflow-x-auto overscroll-x-contain bg-[#F2A03D]"
        >
          {slides.map((s, idx) => (
            s.video ? (
              <video
