...
export default function D100ProductPage() {
  ...
  const product = portableProductModels.find((item) => item.id === requestedModel) ?? defaultPortableProduct;
  ...
  useEffect(() => {
    document.title = `${product.name} Portable Power Station — Product Details | Solvio`;
  }, [product.name]);

  const selectModel = (event) => {
    const modelId = event.target.value;
    setSearchParams(modelId === defaultPortableProduct.id ? {} : { model: modelId }, { replace: true });
  };
          ...
          <div className="container-x">
            ...
            <div className="mt-7 grid gap-10 lg:grid-cols-[minmax(0,1.25fr)_minmax(360px,0.75fr)] xl:gap-16">
              ...
              <aside className="self-start lg:sticky lg:top-32">
                ...
                <div className="mt-7">
                  <label htmlFor="capacity-selector" className="font-mono text-[11px] font-bold uppercase tracking-[0.18em] text-ink/50">
                    Choose capacity
                  </label>
                  <div className="relative mt-2">
                    ...
                    >
                      {portableProductModels.map((model) => (
                ...
                <div className="mt-5 flex items-end justify-between gap-6 border-y border-ink/10 py-5">
                  <div>
                    ...
                    <p className="mt-1 font-display text-2xl font-extrabold text-ink">{product.capacity}</p>
                  </div>
