...
import { d100Product } from '../data/d100';
...
function MediaPlaceholder({ item, featured = false }) {
  return (
    <div
      ...
      aria-label={`Image placeholder ${String(item.id).padStart(2, '0')}: ${item.label}`}
    >
      ...
      <span className="relative w-fit rounded-full bg-ink px-3 py-1.5 font-mono text-[11px] font-bold uppercase tracking-[0.18em] text-white">
        Image {String(item.id).padStart(2, '0')}
      </span>
...
export default function D100ProductPage() {
  usePageMeta('/portable-system/d100');
  const [featured, ...supporting] = d100Product.media.slice(0, 4);
      ...
      <main>
            ...
            <div className="mt-7 grid gap-10 lg:grid-cols-[minmax(0,1.25fr)_minmax(360px,0.75fr)] xl:gap-16">
              <div className="min-w-0">
                <div className="no-scrollbar flex snap-x snap-mandatory gap-4 overflow-x-auto pb-3 lg:grid lg:grid-cols-3 lg:overflow-visible lg:pb-0">
                  ...
                  {supporting.map((item) => (
                    <div key={item.id} className="w-[78%] shrink-0 sm:w-[46%] lg:w-auto">
                ...
                <p className="mt-2 text-center font-mono text-[10px] uppercase tracking-[0.16em] text-ink/45 lg:hidden">
                  Swipe to view image positions 01–04
                </p>
              </div>

              <aside className="self-start lg:sticky lg:top-32">
                <p className="eyebrow">{d100Product.category}</p>
                <h1 className="mt-3 font-display text-5xl font-black tracking-[-0.045em] text-ink sm:text-6xl">
                  {d100Product.name}
                </h1>
                <p className="mt-3 font-display text-xl font-semibold text-[#477296]">{d100Product.tagline}</p>

                <div className="mt-7 flex items-end justify-between gap-6 border-y border-ink/10 py-5">
                  <div>
                    <p className="font-mono text-[11px] uppercase tracking-[0.18em] text-ink/45">Capacity</p>
                    <p className="mt-1 font-display text-2xl font-extrabold text-ink">{d100Product.capacity}</p>
                  </div>
                  <div className="text-right">
                    <p className="font-mono text-[11px] uppercase tracking-[0.18em] text-ink/45">From</p>
                    <p className="mt-1 font-display text-3xl font-black text-price">{baht(d100Product.price)}</p>
                  </div>
                </div>

                <p className="mt-6 text-base leading-relaxed text-ink/75">{d100Product.description}</p>

                <ul className="mt-6 space-y-3">
                  {d100Product.highlights.map((item) => (
                ...
                <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
                  <a
                    href="https://wa.me/66843488428?text=Hi%20Solvio%20%E2%80%94%20I%27d%20like%20more%20information%20about%20the%20D100"
                    target="_blank"
                    rel="noreferrer"
                    className="btn-primary"
                  >
                    <MessageCircle size={18} /> Ask about D100
                  </a>
        ...
        <section id="details" className="bg-[#FFF7E9] py-16 sm:py-24">
          <div className="container-x space-y-16 sm:space-y-24">
            {d100Product.sections.map((section, index) => {
              const media = d100Product.media.find((item) => item.id === section.mediaId);
              return (
                <div key={section.title} className="grid items-center gap-9 lg:grid-cols-2 lg:gap-16">
          ...
          <div className="container-x grid gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:gap-20">
            <div>
              ...
              <h2 className="mt-3 font-display text-4xl font-black tracking-[-0.035em] text-ink sm:text-5xl">
                Clearly labelled. Easy to understand.
              </h2>
              <p className="mt-5 max-w-xl text-base leading-relaxed text-ink/65">
                These are the connection labels visible on the current D100 media. Ask Solvio for the latest approved specification sheet if you need additional technical values.
              </p>
            </div>

            <div>
              <dl className="overflow-hidden rounded-[1.5rem] border border-ink/10">
                {d100Product.connections.map((connection, index) => (
                  <div
                  ...
                  >
                    <dt className="font-display font-bold text-ink">{connection.label}</dt>
                    <dd className="text-right font-mono text-sm text-ink/60">{connection.value}</dd>
                  </div>
                ))}
              </dl>
        ...
        <section className="bg-surface py-16 sm:py-24">
          <div className="container-x max-w-5xl">
            ...
            <div className="mt-10 space-y-3">
              {d100Product.faqs.map((item) => (
                <details key={item.question} className="group rounded-2xl border border-ink/10 bg-white px-5 py-1 shadow-soft sm:px-7">
